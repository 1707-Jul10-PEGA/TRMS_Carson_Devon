...
CREATE OR REPLACE TRIGGER INSERT_TRF_TRIGGER
    ...
    BEGIN
        ...
        IF (:NEW.BC_ID IS NULL) THEN
            ...
            SELECT B.E_ID INTO :NEW.BC_ID FROM BENCO B WHERE B.UNCLAIMED_FORMS = (SELECT MAX(B2.UNCLAIMED_FORMS) FROM BENCO B2);
            UPDATE BENCO B SET B.UNCLAIMED_FORMS = (SELECT COUNT(TRF_ID) FROM TRF WHERE ACTIVE = 1 AND B.E_ID != BC_ID);
        END IF;
            ...
            JOB_ACTION => '
                BEGIN
                    UPDATE REIMBURSEMENT R
                        SET R.AVAILABLE_REIMBURSEMENT = 1000.00 - (SELECT PENDING_REIMBURSEMENTS FROM REIMBURSEMENT R2 WHERE R.E_ID = R2.E_ID);
                END;',
...
INSERT INTO TRF (E_ID, EV_ID, BC_ID, JUSTIFICATION) VALUES (4, 1, 3, 'Yet another test TRF');
